...
async def process_start(message: types.Message):
    try: 
        cursor.execute("SELECT * FROM command WHERE user_id={}".format(message.from_user.id))
        if cursor.fetchone() is None:
            cursor.execute("INSERT INTO command VALUES ({},'{}','{}','{}','{}',{},{},{},'{}',{},{},{})".format(message.from_user.id, 'location_one', 'location_two', '', '', 30, 0, 0, '', 0, 0, 0))
        else:
            cursor.execute("UPDATE command SET current_location='{}', next_location='{}', coins={}, artifact={}, last_hint_id={}, end_date={}, hidden_location='{}', end_game={} WHERE user_id={}".format('location_one', 'location_two', 30, 0, 0, 0, '', 0, message.from_user.id))
        conn.commit()
    except Exception:
        cursor.execute("CREATE TABLE command (user_id INTEGER, current_location TEXT, next_location TEXT, status TEXT, color TEXT, coins INTEGER, artifact INTEGER, last_hint_id INTEGER, hidden_location TEXT, start_time INTEGER, end_date INTEGER, end_game INTEGER)")
        cursor.execute("INSERT INTO command VALUES ({},'{}','{}','{}','{}',{},{},{},'{}',{},{},{})".format(message.from_user.id, 'location_one', 'location_two', '', '', 30, 0, 0, '', 0, 0, 0))
        conn.commit()
...
async def process_check_command(message: types.Message):
    cursor.execute("UPDATE command SET color='{}' WHERE user_id={}".format(message.text, message.from_user.id))
    conn.commit()
...
async def process_send_chat_link(message: types.Message):
    cursor.execute("UPDATE command SET status='{}' WHERE user_id={}".format(message.text, message.from_user.id))
    conn.commit()
    cursor.execute("SELECT color FROM command WHERE user_id={}".format(message.from_user.id))
    color = re.sub(r'[,\'\)\(]', '', str(cursor.fetchone()))
...
async def process_send_preview_message(message: types.Message):
    ...
    try:
        cursor.execute(sql_get_chat_id)
        location_data = cursor.fetchone()
    except Exception:
        cursor.execute("CREATE TABLE locations (chat_id INTEGER, location_color TEXT, team_map TEXT, location_one TEXT, location_two TEXT, location_three TEXT, location_four TEXT, location_five TEXT, location_six TEXT, location_seven TEXT, location_eight TEXT, location_nine TEXT, location_ten TEXT, location_eleven TEXT)")
    ...
    cursor.execute("UPDATE command SET start_time={} WHERE user_id={}".format(calendar.timegm(datetime.utcnow().timetuple()), message.from_user.id))
    conn.commit()
...
async def process_send_start_location_data(message: types.Message):
    sql_get_location_data = "SELECT chat_id, team_map, location_one \
                               ...
                               JOIN command v_com on (v_com.color = v_loc.location_color) \
                              WHERE user_id={}".format(message.from_user.id)
    cursor.execute(sql_get_location_data)
    data = re.split(r',', re.sub(r'[\)\(]', '', str(cursor.fetchone())), 2)
    chat_id = data[0]
...
async def process_check_start_location(message: types.Message):
    ...
    cursor.execute("UPDATE command SET last_hint_id={} WHERE user_id={}".format(message_id.message_id, message.from_user.id))
    conn.commit()
...
async def process_send_start_later_location(message: types.Message):
    cursor.execute("SELECT current_location FROM command WHERE user_id={}".format(message.from_user.id))
    current_location = re.sub(r'[,\)\(\']', '', str(cursor.fetchone()))
    sql_get_location_number = "SELECT " + current_location + " \
                                 FROM locations v_loc \
                                 JOIN command v_com on (v_com.color = v_loc.location_color) \
                                WHERE user_id={}".format(message.from_user.id)
    cursor.execute(sql_get_location_number)
...
async def process_send_legend(message: types.Message):
    sql_get_chat_id = "SELECT v_loc.chat_id, v_com.current_location, v_com.hidden_location, v_com.last_hint_id, v_com.end_game \
                        ...
                        WHERE user_id={}".format(message.from_user.id)
    cursor.execute(sql_get_chat_id)
    data = re.split(r',', re.sub(r'[\)\(]', '', str(cursor.fetchone())))
    ...
    if last_message_id is not 0:
        ...
        cursor.execute("UPDATE command SET last_hint_id={} WHERE user_id={}".format(0, message.from_user.id))
        conn.commit()
    ...
    if message.text == config.blitz:
        if re.search(config.blitz, hidden_location) is None:
            ...
            if re.search(config.scientist, hidden_location) is None:
                cursor.execute("UPDATE command SET hidden_location='{}' WHERE user_id={}".format(config.blitz, message.from_user.id))
            else:
                cursor.execute("UPDATE command SET hidden_location='{}' WHERE user_id={}".format(config.scientist + ';' + config.blitz, message.from_user.id))
            conn.commit()
        else:
    ...
    if message.text == config.scientist:
        if re.search(config.scientist, hidden_location) is None:
            ...
            if re.search(config.blitz, hidden_location) is None:
                cursor.execute("UPDATE command SET hidden_location='{}' WHERE user_id={}".format(config.scientist, message.from_user.id))
            else:
                cursor.execute("UPDATE command SET hidden_location='{}' WHERE user_id={}".format(config.blitz + ';' + config.scientist, message.from_user.id))
            conn.commit()
        else:
    ...
    if message.text == config.say_continue_location:
        sql_get_location_number = "SELECT " + current_location + " \
                                    ...
                                    WHERE user_id={}".format(message.from_user.id)
        cursor.execute(sql_get_location_number)
        current_location = re.sub(r'[\)\(\']', '', str(cursor.fetchone()))[:-1]
...
async def process_send_additional_location_data(call: types.CallbackQuery):
    ...
    if call.data == 'btnGetTask12':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_dop_blitz1, option=config.options_task_blitz1.get('option'), answer=config.options_task_blitz1.get('answer'), location_number=current_location)
    ...
    if call.data == 'btnGetTask13':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_dop_scientist, open_period=420, option=config.options_task_scientist.get('option'), answer=config.options_task_scientist.get('answer'), location_number=current_location)
    ...
    if call.data == 'btnGetTask1':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_one+'_'+config.task_number1, option=config.options_task_one1.get('option'), answer=config.options_task_one1.get('answer'), hint_number=kb.kb_HintOne_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask2':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_two+'_'+config.task_number1, option=config.options_task_two1.get('option'), answer=config.options_task_two1.get('answer'), hint_number=kb.kb_HintTwo_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask3':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_three+'_'+config.task_number1, option=config.options_task_three1.get('option'), answer=config.options_task_three1.get('answer'), hint_number=kb.kb_HintThree_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask4':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_four+'_'+config.task_number1, option=config.options_task_four1.get('option'), answer=config.options_task_four1.get('answer'), hint_number=kb.kb_HintFour_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask5':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_five+'_'+config.task_number1, option=config.options_task_five1.get('option'), answer=config.options_task_five1.get('answer'), hint_number=kb.kb_HintFive_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask6':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_six+'_'+config.task_number1, option=config.options_task_six1.get('option'), answer=config.options_task_six1.get('answer'), hint_number=kb.kb_HintSix_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask7':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_seven+'_'+config.task_number1, option=config.options_task_seven1.get('option'), answer=config.options_task_seven1.get('answer'), hint_number=kb.kb_HintSeven_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask8':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_eight+'_'+config.task_number1, option=config.options_task_eight1.get('option'), answer=config.options_task_eight1.get('answer'), hint_number=kb.kb_HintEight_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask9':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_nine+'_'+config.task_number1, option=config.options_task_nine1.get('option'), answer=config.options_task_nine1.get('answer'), hint_number=kb.kb_HintNine_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask10':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_ten+'_'+config.task_number1, option=config.options_task_ten1.get('option'), answer=config.options_task_ten1.get('answer'), hint_number=kb.kb_HintTen_1, location_number=next_location)
    ...
    if call.data == 'btnGetTask11':
        await quiz_poll(user_id=call.message.chat.id, delete_message=False, chat_id=chat_id, task=config.task_eleven+'_'+config.task_number1, open_period=480, option=config.options_task_eleven.get('option'), answer=config.options_task_eleven.get('answer'))
        await bot.send_message(os.environ.get('ADMIN_CHAT_ID'), color_command + config.admin_start_location + config.location_eleven)

async def quiz_poll(user_id=0, delete_message=True, last_hint_id=None, correct_answer=None, coins_command=None, coin=None, artifact=None, artifact_command=None, chat_id=0, end_date=None, task=None, open_period=None, option=None, answer=None, hint_number=None, location_number=None):
    try:
        cursor.execute("SELECT * FROM poll WHERE user_id={}".format(user_id))        
        data = cursor.fetchone()
        if data is None:
            cursor.execute("INSERT INTO poll VALUES ({},{},'{}',{})".format(user_id, 0, '', 0))
            conn.commit()
    except Exception:
        cursor.execute("CREATE TABLE poll (user_id INTEGER, poll_id INTEGER, question TEXT, correct_answer INTEGER)")
        cursor.execute("INSERT INTO poll VALUES ({},{},'{}',{})".format(user_id, 0, '', 0))
        conn.commit()

    if delete_message is True and int(last_hint_id) is not 0:
        await bot.delete_message(user_id, last_hint_id)
        cursor.execute("UPDATE command SET last_hint_id={} WHERE user_id={}".format(0, user_id))
        conn.commit()

    if correct_answer is False:
        await bot.send_message(user_id, config.coins_number + str(coins_command))
    if correct_answer is True:
        cursor.execute("UPDATE command SET coins={} WHERE user_id={}".format(coins_command + coin, user_id))
        if artifact is not None:
            cursor.execute("UPDATE command SET artifact={} WHERE user_id={}".format(artifact_command + 1, user_id))
    ...
    if location_number is not None:
        ...
        cursor.execute(sql_insert_location)
        conn.commit()
    ...
    if task is not None:
        ...
        if open_period is None:
            if 'dop' in task:
                ...
                cursor.execute("UPDATE command SET end_date={} WHERE user_id={}".format(end_date, user_id))
            else:
                if end_date is None:
                ...
                cursor.execute("UPDATE command SET end_date={}, last_hint_id={} WHERE user_id={}".format(end_date, last_hint_id.message_id, user_id))
        else:
        ...
        cursor.execute("UPDATE poll SET poll_id={}, question='{}', correct_answer={} WHERE user_id={}".format(poll_data.poll.id, task, answer, user_id))
        conn.commit()
...
async def process_poll_handler(quiz_answer: types.Poll):
    ...
    cursor.execute(sql_get_poll_data)
    poll_data = re.split(r',', re.sub(r'[\)\(]', '', str(cursor.fetchone())))
    ...
    if question == config.task_one+'_'+config.task_number1:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_one1.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_one+'_'+config.task_number2, option=config.options_task_one2.get('option'), answer=config.options_task_one2.get('answer'), hint_number=kb.kb_HintOne_2)
    if question == config.task_one+'_'+config.task_number2:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_one2.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_one+'_'+config.task_number3, option=config.options_task_one3.get('option'), answer=config.options_task_one3.get('answer'), hint_number=kb.kb_HintOne_3)
    if question == config.task_one+'_'+config.task_number3:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_one3.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_one+'_'+config.task_number4, option=config.options_task_one4.get('option'), answer=config.options_task_one4.get('answer'), hint_number=kb.kb_HintOne_4)
    if question == config.task_one+'_'+config.task_number4:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_one4.get('coin'), chat_id=chat_id, artifact='artifact_one', artifact_command=artifact_command)
    ...
    if question == config.task_two+'_'+config.task_number1:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_two1.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_two+'_'+config.task_number2, option=config.options_task_two2.get('option'), answer=config.options_task_two2.get('answer'), hint_number=kb.kb_HintTwo_2)
    if question == config.task_two+'_'+config.task_number2:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_two2.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_two+'_'+config.task_number3, option=config.options_task_two3.get('option'), answer=config.options_task_two3.get('answer'), hint_number=kb.kb_HintTwo_3)
    if question == config.task_two+'_'+config.task_number3:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_two3.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_two+'_'+config.task_number4, option=config.options_task_two4.get('option'), answer=config.options_task_two4.get('answer'), hint_number=kb.kb_HintTwo_4)
    if question == config.task_two+'_'+config.task_number4:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_two4.get('coin'), chat_id=chat_id, artifact='artifact_two', artifact_command=artifact_command)
        await bot.send_message(quiz_answer.user.id, config.end_location)
    ...
    if question == config.task_three+'_'+config.task_number4:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_three4.get('coin'), chat_id=chat_id)
        await bot.send_message(quiz_answer.user.id, config.end_location)
    ...
    if question == config.task_nine+'_'+config.task_number4:
        await quiz_poll(user_id=quiz_answer.user.id, last_hint_id=last_hint_id, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_nine4.get('coin'), chat_id=chat_id, artifact='artifact_nine', artifact_command=artifact_command)
        await bot.send_message(quiz_answer.user.id, config.end_location)
    ...
    if question == config.task_dop_blitz3:
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_blitz3.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_dop_blitz4, option=config.options_task_blitz4.get('option'), answer=config.options_task_blitz4.get('answer'))
    if question == config.task_dop_blitz4:
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_blitz4.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_dop_blitz5, option=config.options_task_blitz5.get('option'), answer=config.options_task_blitz5.get('answer'))
    if question == config.task_dop_blitz5:
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_blitz5.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_dop_blitz6, option=config.options_task_blitz6.get('option'), answer=config.options_task_blitz6.get('answer'))
    if question == config.task_dop_blitz6:
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_blitz6.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_dop_blitz7, option=config.options_task_blitz7.get('option'), answer=config.options_task_blitz7.get('answer'))
    if question == config.task_dop_blitz7:
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_blitz7.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_dop_blitz8, option=config.options_task_blitz8.get('option'), answer=config.options_task_blitz8.get('answer'))
    ...
    if question == config.task_dop_blitz9:
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_blitz9.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_dop_blitz10, option=config.options_task_blitz10.get('option'), answer=config.options_task_blitz10.get('answer'))
    ...
    if question == config.task_dop_blitz11:
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_blitz11.get('coin'), chat_id=chat_id, end_date=end_date, task=config.task_dop_blitz12, option=config.options_task_blitz12.get('option'), answer=config.options_task_blitz12.get('answer'))
    if question == config.task_dop_blitz12:
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_blitz12.get('coin'), chat_id=chat_id)
        await bot.send_message(quiz_answer.user.id, config.end_location)
    ...
    if question == config.task_dop_scientist:
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_scientist.get('coin'), chat_id=chat_id)
        await bot.send_message(quiz_answer.user.id, config.end_location)
    ...
    if question == config.task_eleven+'_'+config.task_number1:
        ...
        await quiz_poll(user_id=quiz_answer.user.id, delete_message=False, correct_answer=correct_answer, coins_command=coins_command, coin=config.options_task_eleven.get('coin'), chat_id=chat_id)
        await bot.send_message(quiz_answer.user.id, config.end_location)

        cursor.execute("SELECT color, coins, artifact, start_time FROM command WHERE user_id={}".format(quiz_answer.user.id))
        command_data = re.split(r',', re.sub(r'[\)\(]', '', str(cursor.fetchone())))
        ...
        cursor.execute("UPDATE command SET hidden_location='{}', end_game={} WHERE user_id={}".format(config.scientist + ';' + config.blitz, 1, quiz_answer.user.id))
        conn.commit()

async def send_hint_data(call, last_hint_id, coins_command, hint_data, hint_cost):
    if coins_command >= hint_cost:
        cursor.execute("UPDATE command SET coins={} WHERE user_id={}".format(coins_command - hint_cost, call.message.chat.id))
        conn.commit()
        ...
        if len(inline_list) == 0:
            ...
            cursor.execute("UPDATE command SET last_hint_id={} WHERE user_id={}".format(0, call.message.chat.id))
            conn.commit()
    else:
...
async def process_check_task(call: types.CallbackQuery):
    cursor.execute("SELECT last_hint_id, coins FROM command WHERE user_id={}".format(call.message.chat.id))
    command_data = re.split(r',', re.sub(r'[\'\)\(]', '', str(cursor.fetchone())))
    last_hint_id = re.sub(r' ', '', command_data[0])
    coins_command = int(re.sub(r' ', '', command_data[1]))

    if call.data == 'btnHintOne_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one1, 1)
    if call.data == 'btnHintOne_2':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one2, 3)
    if call.data == 'btnHintOne_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one3, 5)
    if call.data == 'btnHintOne_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one4, 1)
    if call.data == 'btnHintOne_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one5, 3)
    if call.data == 'btnHintOne_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one6, 5)
    if call.data == 'btnHintOne_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one7, 1)
    if call.data == 'btnHintOne_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one8, 3)
    if call.data == 'btnHintOne_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one9, 5)
    if call.data == 'btnHintOne_10':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one10, 1)
    if call.data == 'btnHintOne_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one11, 3)
    if call.data == 'btnHintOne_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_one12, 5)

    if call.data == 'btnHintTwo_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two1, 1)
    if call.data == 'btnHintTwo_2':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two2, 3)
    if call.data == 'btnHintTwo_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two3, 5)
    if call.data == 'btnHintTwo_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two4, 1)
    if call.data == 'btnHintTwo_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two5, 3)
    if call.data == 'btnHintTwo_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two6, 5)
    if call.data == 'btnHintTwo_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two7, 1)
    if call.data == 'btnHintTwo_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two8, 3)
    if call.data == 'btnHintTwo_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two9, 5)
    if call.data == 'btnHintTwo_10':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two10, 1)
    if call.data == 'btnHintTwo_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two11, 3)
    if call.data == 'btnHintTwo_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_two12, 5)

    if call.data == 'btnHintThree_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three1, 1)
    if call.data == 'btnHintThree_2':
        await send_hint_data(call, last_hint_id, coins_command, 'hint_three2', 3)
    if call.data == 'btnHintThree_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three3, 5)
    if call.data == 'btnHintThree_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three4, 1)
    if call.data == 'btnHintThree_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three5, 3)
    if call.data == 'btnHintThree_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three6, 5)
    if call.data == 'btnHintThree_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three7, 1)
    if call.data == 'btnHintThree_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three8, 3)
    if call.data == 'btnHintThree_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three9, 5)
    if call.data == 'btnHintThree_10':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three10, 1)
    if call.data == 'btnHintThree_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three11, 3)
    if call.data == 'btnHintThree_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_three12, 5)

    if call.data == 'btnHintFour_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four1, 1)
    if call.data == 'btnHintFour_2':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four2, 3)
    if call.data == 'btnHintFour_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four3, 5)
    if call.data == 'btnHintFour_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four4, 1)
    if call.data == 'btnHintFour_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four5, 3)
    if call.data == 'btnHintFour_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four6, 5)
    if call.data == 'btnHintFour_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four7, 1)
    if call.data == 'btnHintFour_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four8, 3)
    if call.data == 'btnHintFour_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four9, 5)
    if call.data == 'btnHintFour_10':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four10, 1)
    if call.data == 'btnHintFour_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four11, 3)
    if call.data == 'btnHintFour_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_four12, 5)

    if call.data == 'btnHintFive_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five1, 1)
    if call.data == 'btnHintFive_2':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five2, 3)
    if call.data == 'btnHintFive_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five3, 5)
    if call.data == 'btnHintFive_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five4, 1)
    if call.data == 'btnHintFive_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five5, 3)
    if call.data == 'btnHintFive_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five6, 5)
    if call.data == 'btnHintFive_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five7, 1)
    if call.data == 'btnHintFive_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five8, 3)
    if call.data == 'btnHintFive_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five9, 5)
    if call.data == 'btnHintFive_10':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five10, 1)
    if call.data == 'btnHintFive_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five11, 3)
    if call.data == 'btnHintFive_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_five12, 5)

    if call.data == 'btnHintSix_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six1, 1)
    if call.data == 'btnHintSix_2':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six2, 3)
    if call.data == 'btnHintSix_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six3, 5)
    if call.data == 'btnHintSix_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six4, 1)
    if call.data == 'btnHintSix_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six5, 3)
    if call.data == 'btnHintSix_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six6, 5)
    if call.data == 'btnHintSix_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six7, 1)
    if call.data == 'btnHintSix_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six8, 3)
    if call.data == 'btnHintSix_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six9, 5)
    if call.data == 'btnHintSix_10':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six10, 1)
    if call.data == 'btnHintSix_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six11, 3)
    if call.data == 'btnHintSix_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_six12, 5)

    if call.data == 'btnHintSeven_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven1, 1)
    if call.data == 'btnHintSeven_2':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven2, 3)
    if call.data == 'btnHintSeven_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven3, 5)
    if call.data == 'btnHintSeven_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven4, 1)
    if call.data == 'btnHintSeven_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven5, 3)
    if call.data == 'btnHintSeven_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven6, 5)
    if call.data == 'btnHintSeven_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven7, 1)
    if call.data == 'btnHintSeven_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven8, 3)
    if call.data == 'btnHintSeven_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven9, 5)
    if call.data == 'btnHintSeven_10':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven10, 1)
    if call.data == 'btnHintSeven_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven11, 3)
    if call.data == 'btnHintSeven_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_seven12, 5)

    if call.data == 'btnHintEight_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight1, 1)
    if call.data == 'btnHintEight_2':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight2, 3)
    if call.data == 'btnHintEight_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight3, 5)
    if call.data == 'btnHintEight_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight4, 1)
    if call.data == 'btnHintEight_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight5, 3)
    if call.data == 'btnHintEight_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight6, 5)
    if call.data == 'btnHintEight_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight7, 1)
    if call.data == 'btnHintEight_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight8, 3)
    if call.data == 'btnHintEight_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight9, 5)
    if call.data == 'btnHintEight_10':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight10, 1)
    if call.data == 'btnHintEight_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight11, 3)
    if call.data == 'btnHintEight_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_eight12, 5)

    if call.data == 'btnHintNine_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine1, 1)
    if call.data == 'btnHintNine_2':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine2, 3)
    if call.data == 'btnHintNine_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine3, 5)
    if call.data == 'btnHintNine_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine4, 1)
    if call.data == 'btnHintNine_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine5, 3)
    if call.data == 'btnHintNine_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine6, 5)
    if call.data == 'btnHintNine_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine7, 1)
    if call.data == 'btnHintNine_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine8, 3)
    if call.data == 'btnHintNine_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine9, 5)
    if call.data == 'btnHintNine_10':
        await send_hint_data(call, last_hint_id, coins_command, 'hint_nine10', 1)
    if call.data == 'btnHintNine_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine11, 3)
    if call.data == 'btnHintNine_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_nine12, 5)

    if call.data == 'btnHintTen_1':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten1, 1)
    if call.data == 'btnHintTen_2':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten2, 3)
    if call.data == 'btnHintTen_3':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten3, 5)
    if call.data == 'btnHintTen_4':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten4, 1)
    if call.data == 'btnHintTen_5':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten5, 3)
    if call.data == 'btnHintTen_6':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten6, 5)
    if call.data == 'btnHintTen_7':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten7, 1)
    if call.data == 'btnHintTen_8':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten8, 3)
    if call.data == 'btnHintTen_9':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten9, 5)
    if call.data == 'btnHintTen_10':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten10, 1)
    if call.data == 'btnHintTen_11':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten11, 3)
    if call.data == 'btnHintTen_12':
        await send_hint_data(call, last_hint_id, coins_command, config.hint_ten12, 5)
